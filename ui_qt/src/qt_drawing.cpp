...
namespace dak
{
   namespace ui_qt
   {
      ...
      QBrush qt_drawing::get_brush() const
      {
         const auto& co = get_color();
         return QBrush(convert(co));
      }

      static void clear_background(drawing& drw)
      {
         drw.set_color(color::white());
         drw.fill_polygon(polygon::from_rect(drw.get_bounds()).apply(drw.get_transform().invert()));
      }

      static bool begin_drawing_tiling(drawing& drw, const tiling::tiling& tiling, const color& co, int copy_count)
      {
      ...
      static void end_drawing_tiling(drawing& drw)
      {
